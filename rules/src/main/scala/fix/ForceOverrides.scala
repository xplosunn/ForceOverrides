...
final class ForceOverrides extends SemanticRule("fix.ForceOverrides") {

  private def getMethods(
    semanticType: SemanticType
  )(implicit doc: SemanticDocument): List[(String, MethodSignature)] =
    semanticType match {
  ...
  private def paramListListEq(
    l1: List[List[SymbolInformation]],
    l2: List[List[SymbolInformation]],
  ): Boolean =
    (l1, l2) match {
  ...
  private def defnFix(t: Tree)(implicit doc: SemanticDocument): List[Patch] = {
    ...
    t.collect { case t @ Defn.Def(mods, name, tparams, paramss, typeOpt, term) =>
      t.symbol.info match {
        case Some(info) =>
          info.signature match {
            case signature @ MethodSignature(typeParameters, parameterLists, returnType) =>
              if (
                parentMethods.exists { case (methodName, methodSignature) =>
